...
export class AddCandidatePageComponent implements OnInit, DoCheck {
    ...
    ngOnInit(): void {
        ...
        this.candidateId = this.router.snapshot.queryParamMap.get('id');
            if(!this.candidateId){
                this.edition = false
                setTimeout(() => {
                    this.title.setTitle('Добавление кандидата')
                }, 10)
            } else {
                this.edition = true
                setTimeout(() => {
                    this.title.setTitle('Редактирование кандидата')
                }, 10)
    ...
    submit() {
        ...
        let candidateId: string | null = this.router.snapshot.queryParams.id
        this.candidatesService.putCandidate(this.formToPost, candidateId).subscribe(put => {
    ...
    fetchCandidate(id: null | string) {
        this.candidatesService.getCandidate(id).subscribe(src => {
            // if (!src.results) {
            //     this.inBase = false
            //     return
            // }
            this.inBase = true
            this.form.controls.f_i_o.setValue(src.f_i_o)
            this.form.controls.phone_number.setValue(src.phone_number)
            this.form.controls.salary.setValue(src.salary)
            this.form.controls.job_position.setValue(src.job_position)
            this.form.controls.description.setValue(src.description)
            this.form.controls.feedback.setValue(src.feedback)
            this.form.controls.birth_date.setValue(src.birth_date)
            this.form.controls.place_of_employment.setValue(src.place_of_employment)
            for (let t of src.candidatetechnology_set) {
                this.setLevel(t.knowledge_level, t.technology_id)
            }
        })
    }
